...
router.post('/login', function (req, res) {
    ...
    if (errors.array().length == 0) {
        ...
        res.json({
            ...
            message: trans_message.INVALID_DATA,
            data: errors.array()
        });
...
router.post('/register', function (req, res) {
    if (req.headers.locale == 'ar') {
        var trans_message = ar_messages; // get arabic message
    } else if (req.headers.locale == 'en') {
        var trans_message = messages; // get english message
    } else {
        var trans_message = messages;
    }
    var errors = validationResult(req);
                ...
                authenticationService.check_user(creqentials.email, creqentials.password , creqentials.first_name, creqentials.last_name, creqentials.phone, creqentials.user_type).then(user => {
                    ...
                    if (user == null) {
                        res.json({
                            status: statics.STATUS_FAILURE,
                            code: codes.FAILURE,
                            message: trans_message.EMAIL_REGISTERED,
                            data: null
                        });
                    } else {
                        var userdata = {
    ...
    if (errors.array().length == 0) {
        ...
        if (otp.otp == '') {
            res.json({
                status: statics.STATUS_FAILURE,
                code: codes.FAILURE,
                message: trans_message.EMPTY_FIELD_OTP,
                data: null
            });
        } else {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (!token) {
                    res.json({
                        ...
                        message: trans_message.NO_TOKEN,
                        auth: false,
                        data: null
                    });
                }
                ...
                authenticationService.check_token(headerdata.token).then(user => {
                    ...
                    if (user == null) {
                        ...
                        res.json({
                            status: statics.STATUS_FAILURE,
                            code: codes.FAILURE,
                            message: trans_message.INVALID_TOKEN,
                            data: null
                        });
    ...
    if (errors.array().length == 0) {
        ...
        if (data.first_name == '') {
            ...
            res.json({
                status: statics.STATUS_FAILURE,
                code: codes.FAILURE,
                message: trans_message.EMPTY_FIELD_ADDRESS,
                data: null
            });
        } else if (data.phone == '') {
            res.json({
            ...
            res.json({
                status: statics.STATUS_FAILURE,
                code: codes.FAILURE,
                message: trans_message.EMPTY_FIELD_LAT,
                data: null
            });
            ...
            res.json({
                status: statics.STATUS_FAILURE,
                code: codes.FAILURE,
                message: trans_message.EMPTY_FIELD_LONG,
                data: null
            });
            ...
            res.json({
                status: statics.STATUS_FAILURE,
                code: codes.FAILURE,
                message: trans_message.EMPTY_FIELD_COMPANY,
                data: null
            });
        } else {
...
router.put('/resend_code', function (req, res) {
    ...
    if (errors.array().length == 0) {
        ...
        return new Promise(function (resolve, reject) {
            var email;
            jwt.verify(headerdata.token, config.secret, function(err, decoded) {
                if (err)
                {
                    res.json({
                        status: statics.STATUS_FAILURE,
                ...
                authenticationService.check_email(creqentials.email).then(user => {
                    ...
                    if(!user) {
                            ...
                            res.json({
                                status: statics.STATUS_FAILURE,
                                code: codes.INVALID_DATA,
                                message: trans_message.INVALID_EMAIL,
                                data: user
                            });
    ...
    if (errors.array().length == 0) {
        ...
        if (data.password == '') {
            ...
            return new Promise(function (resolve, reject) {
                ...
                authenticationService.update_pass(data.otp, data.password).then(user => {
