                    ...
                    AccountService.Check(credentials).then(isAccount => {
                        ...
                        if (!isAccount) {
                            AccountService.Create(credentials).then(account => {
                                ...
                                if (account.user_id === credentials.user_id) {
                                    languageService.get_lang(lang, 'SUCCESS').then(msg => {
                                        res.json({
                                            status: statics.STATUS_SUCCESS,
                                            code: codes.SUCCESS,
...
router.put('/update', async function (req, res) {
    ...
    if (errors.array().length == 0) {
        ...
        return new Promise(function (resolve, reject) {
            credentials.user_id = id;
            if (!credentials.account_id || credentials.account_id == '') {
...
router.delete('/delete/:account_id', async function (req, res) {
    var lang = req.headers.language;
    var errors = validationResult(req);
    if (errors.array().length == 0) {
        var credentials = req.body;
        ...
        return new Promise(function (resolve, reject) {
            if (!req.params.account_id || req.params.account_id == '') {
                languageService.get_lang(lang, 'EMPTY_FIELD_ACCOUNT_ID').then(msg => {
                    res.json({
                        status: statics.STATUS_FAILURE,
                        code: codes.FAILURE,
                ...
                return new Promise(function (resolve, reject) {
                    AccountService.Delete(req.params.account_id).then(account => {
                        resolve(account);
